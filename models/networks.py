...
from util.extractor import global_extractor,local_extractor
...
class PatchSampleF(nn.Module):
    ...
    def forward(self, feats, num_patches=64, patch_ids=None, attn_mats=None):
        ...
        for feat_id, feat in enumerate(feats):
            i = i + 1;
            if i < 3:
                # the first two layers use global attention extractor.
                f_s, i_d, a_s = global_extractor(self, feat_id, feat, feats, num_patches, patch_ids, attn_mats)
                return_feats.append(f_s)
                return_ids.append(i_d)
                return_mats.append(a_s)
            else:
               # the last two layers use local attention extractor.
                f_s, i_d, a_s = local_extractor(self, feat_id, feat, feats, num_patches, patch_ids, attn_mats)
                return_feats.append(f_s)
                return_ids.append(i_d)
                return_mats.append(a_s)
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        model = [nn.ReflectionPad2d(3),
                 ...
                 nn.Conv2d(input_nc, input_nc, kernel_size=7, padding=0, bias=use_bias, groups=input_nc),
                 nn.Conv2d(input_nc, ngf, kernel_size=1, groups=1),
                 norm_layer(ngf),
                 nn.ReLU(True)]
        ...
        for i in range(n_downsampling):  # add downsampling layers
            ...
            if(no_antialias):
                model += [
                          ...
                          # nn.Conv2d(ngf * mult, ngf * mult, kernel_size=3, stride=2, padding=1, bias=use_bias, groups=ngf * mult),
                          # nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=1, groups=1),
                          norm_layer(ngf * mult * 2),
                          nn.ReLU(True)]
            else:
                model += [
                          nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=1, padding=1, bias=use_bias),
                          # nn.Conv2d(ngf * mult, ngf * mult, kernel_size=3, stride=1, padding=1, bias=use_bias, groups=ngf * mult),
                          # nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=1, groups=1),
                          norm_layer(ngf * mult * 2),
                          nn.ReLU(True),
